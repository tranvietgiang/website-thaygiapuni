...
import { saveContent as persistContent } from "../../../data/siteContent.js";
...
function Panel({ title, description, icon: Icon, children, tabId, activeTab }) {
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-4 backdrop-blur md:px-8">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                className={`hidden items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold transition md:flex ${
                  hasUnsavedChanges
        ...
        <section className="space-y-6 px-4 py-6 md:px-8">
          ...
          <style>
            {`
              .dashboard-tab-panels > section { display: none; }
              .dashboard-tab-hero > section:nth-of-type(1),
              .dashboard-tab-stats > section:nth-of-type(2),
              .dashboard-tab-programs > section:nth-of-type(3),
              .dashboard-tab-teachers > section:nth-of-type(4),
              .dashboard-tab-contact > section:nth-of-type(5) { display: block; }
            `}
          </style>

          <div className={`dashboard-tab-panels dashboard-tab-${activeTab}`}>
            <Panel
              title="Hero"
            ...
            >
              ...
              <div className="grid gap-3">
                {current.stats.map((item, index) => (
                  <div
                    className="grid gap-3 rounded-lg border border-slate-100 bg-slate-50 p-4 md:grid-cols-[180px_1fr]"
                    key={index}
                  >
                    <Field
                      label="Giá trị"
                      value={item.value}
                      onChange={(value) =>
                        update(["stats", index, "value"], value)
                      }
                    />
                    <Field
                      label="Nhãn"
                      value={item.label}
                      onChange={(value) =>
                        update(["stats", index, "label"], value)
                      }
                    />
                  </div>
                ))}
            ...
            <Panel
              ...
              description="Card khóa học gồm tên, mô tả, tag và ảnh dọc giống trang UNI."
              icon={BookOpen}
            >
              <div className="mb-4 grid gap-4 lg:grid-cols-2">
                <Field
            ...
            <Panel title="Liên hệ và footer" icon={Save}>
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <Field
                  ...
                  onChange={(value) =>
                    update(["contact", "description"], value)
                  }
                />
                <Field
                  textarea
